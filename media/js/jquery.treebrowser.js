...
(function($) {
  $.extend({
    indiciaTreeBrowser: new function() {
      ...
      */
      this.construct = function(options) {
        ...
        $.extend(this.settings, $.indiciaTreeBrowser.defaults, options);        
        load(this);
      }
      ...
      function load(container, id) {
    	...
    	$("#"+container.settings.valueControl).val(id);    	    	
    	if (id==undefined) {
    	  id='NULL';
    	}    		  
    	var extras = '';
    		  ...
    		  if (response.length) {    			
    		    ...
    		    $.each(response, function(idx, record) {    			
                  ...
                  for (var item in record) {
                    node = node.replace('{'+item+ '}', record[item])
                  }    			
    			  var current = $('<li class="ui-state-default"/>').attr("id", record[container.settings.valueField] || "").html(node).appendTo(list);
    		    ...
    		    if (container.settings.singleLayer && id!='NULL') {
    		      ...
    		      var back = $('<li class="ui-state-default">'+container.settings.backCaption+"</li>").click(function() {
   ...
   */
  $.fn.extend({
	indiciaTreeBrowser: $.indiciaTreeBrowser.construct
  });
})(jQuery);
