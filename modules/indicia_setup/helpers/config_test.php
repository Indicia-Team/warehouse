...
class config_test {
  ...
  private static function check_dBase(&$messages, $problems_only) {
    if (!function_exists('dbase_open')) {
      ...
      array_push($messages, array(
        'title' => 'dBase Library',
        'description' => 'The dBase library is not installed on this web server. This is required to enable upload of SHP files containing location boundaries, but '.
            ...
            'webserver process. If you compiled PHP yourself, you must compile PHP with dbase support by using the --enable-dbase configuration option.',
        'success' => true,
        'warning' => true
      ));
    } elseif (!$problems_only) {
      array_push($messages, array(
        'title' => 'dBase Library',
        'description' => 'The dBase library is installed.',
        'success' => true
      ));
    }
  }
...
boundaries, but does not stop Indicia working. To fix this for Windows servers, find your php.ini file in the PHP
installation folder and find the line <strong>;extension=php_zip.dll</strong>. Remove the semi-colon from the start of
the line and save the file, then restart your webserver. For Linux systems, you must compile PHP with zip support by
using the --enable-zip configuration option. Please pass this information to the administrator of your webserver if you
...
DESC;
    ...
    elseif (!$problems_only) {
      array_push($messages, array(
        'title' => 'Zip Library',
        'description' => 'The Zip library is installed.',
        'success' => TRUE
      ));
    }
  }
