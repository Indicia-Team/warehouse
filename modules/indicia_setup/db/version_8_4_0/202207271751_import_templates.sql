...
(
  ...
  CONSTRAINT fk_import_templates_group FOREIGN KEY (group_id)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT fk_import_templates_website FOREIGN KEY (website_id)
      REFERENCES websites (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
);

COMMENT ON TABLE import_templates IS 'Predefined templates for imports.';
COMMENT ON COLUMN import_templates.title IS 'User defined title for the template';
...
COMMENT ON COLUMN import_templates.group_id IS 'Foreign key to the website this template is for.';
