ALTER TABLE occurrence_comments ADD COLUMN confidential boolean;
COMMENT ON COLUMN occurrence_comments.confidential IS
  'Is this occurrence_comment confidential?';

DROP VIEW detail_occurrence_comments;
...
CREATE OR REPLACE VIEW list_occurrence_comments AS 
 SELECT oc.id, oc.comment, oc.occurrence_id, oc.email_address, oc.updated_on, COALESCE(oc.person_name, ((p.surname::text || ', '::text) || p.first_name::text)::character varying) AS person_name,
    u.username, o.website_id, oc.record_status, oc.record_substatus, oc.query, oc.confidential
   FROM occurrence_comments oc
   JOIN occurrences o ON o.id = oc.occurrence_id AND o.deleted = false
