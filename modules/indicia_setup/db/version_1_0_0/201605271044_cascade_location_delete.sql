...
$BODY$
  BEGIN
    IF (OLD.deleted = false AND NEW.deleted = true) THEN
      ...
      UPDATE cache_occurrences_functional SET location_id=null, location_name=COALESCE(location_name, OLD.name) where location_id=OLD.id;
      UPDATE cache_samples_functional SET location_id=null, location_name=COALESCE(location_name, OLD.name) where location_id=OLD.id;
    END IF;
  RETURN OLD;
END;
$BODY$
