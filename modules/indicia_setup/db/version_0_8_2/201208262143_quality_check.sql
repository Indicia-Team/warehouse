-- Function: quality_check(character varying, character, character)

-- DROP FUNCTION quality_check(character varying, character, character);
...
r=
  ...
  ('!R'='!D' AND record_status NOT IN ('D')) OR
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION quality_check(character varying, character, character) OWNER TO indicia_user;
COMMENT ON FUNCTION quality_check(character varying, character, character) IS 'Implements a standard quality check for a record, based on the record_status and certainty. Quality can be requested to be V (verified), C (at least certain), L (at least likely), !D (not dubious or rejected), !R (not rejected).';
