...
select 
  ...
  oa.condition_id, cond.term as condition, oa.position_id, pos.term as position,
  oa.impact_id, impact.term as impact
...
left join cache_termlists_terms cond on cond.id=oa.condition_id
...
select 
  ...
  oa.association_type_id, atype.term as association_type, oa.part_id, part.term as part, 
  oa.condition_id, cond.term as condition, oa.position_id, pos.term as position,
  oa.impact_id, impact.term as impact, oa.comment,
  oa.created_by_id, c.username AS created_by, oa.updated_by_id, u.username AS updated_by
from occurrence_associations oa
join cache_occurrences cofrom on cofrom.id=oa.from_occurrence_id
join cache_occurrences coto on coto.id=oa.to_occurrence_id
join cache_termlists_terms atype on atype.id=oa.association_type_id
