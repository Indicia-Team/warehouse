...
BEGIN
	...
	IF (tableName = 'samples') THEN
 		...
 		ELSE
 			...
 			SELECT survey_id INTO surveyID FROM indicia.samples WHERE id = tableRow.sample_id;
	    	RAISE INFO 'audit.get_indicia_websites : Looking for surveyID %',surveyID;
		END IF;
 		-- we have been given a child record, so need to look up the sample
 		SELECT ARRAY[website_id] INTO websiteIDs FROM indicia.surveys WHERE id = surveyID;
    ELSIF (tableName = 'occurrences') THEN
 		...
 		ELSE
 			SELECT ARRAY[website_id] INTO websiteIDs FROM indicia.occurrences WHERE id = tableRow.occurrence_id;
 		END IF;
    ELSIF (tableName = 'locations') THEN
 		...
 		END IF;
 		-- we have been given a child record, so need to look up the sample
 		SELECT array(SELECT website_id FROM indicia.locations_websites WHERE location_id = locatonID AND deleted = 'f') INTO websiteIDs;
    END IF;
