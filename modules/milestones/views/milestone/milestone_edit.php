...
/**
 * @file
 * View template for the milestone edit form.
 *
...
?>
<form class="iform" id="milestones-form" action="<?php echo $url; ?>" method="post">
  <fieldset>
    <legend>Milestone details<?php echo $metadata; ?></legend>
    <?php
    ...
    // without even getting as far as doing the milestone model's title
    // duplciate detection.
    // So to fix this, collect the existing filters from the database so we can
    // compare the titles with the one we create and then do the validation
    // manually.
    $readAuth = data_entry_helper::get_read_auth(0 - $_SESSION['auth_user']->id, kohana::config('indicia.private_key'));
    $existingFilterData = data_entry_helper::get_population_data([
      'table' => 'filter',
      'extraParams' => $readAuth,
      'nocache' => TRUE,
    ]);

    // When we save a milestone when we need to automatically set the filter
    // title as there isn't a separate field to fill this in.
    // Also hide the "who" filter as we don't need this for milestones as they
    // can apply to all users.
    // Also manually do the unique milestone/filter title validation (see note
    // above).
    $jsFilterData = json_encode($existingFilterData);
...
JS;
    ...
    $filterPanelHTML .= data_entry_helper::hidden_text([
      'fieldname' => 'filter:id',
      'default' => html::initial_value($values, 'filter:id'),
    ]);
