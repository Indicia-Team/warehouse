...
class task_import_step {
  ...
  private static function notifyUserOfCompletionAndCleanup($db, $task, $params) {
    ...
    if ($personInfo) {
      ...
      $emailer->send(
        'Import complete',
        "The import you requested has completed successfully.\n\nRegards\nIndicia Team"
      );
    }
    else {
  ...
  private static function notifyUserOfCrash($db, $task, $params, $e) {
    ...
    if ($personInfo) {
      ...
      $emailer->send(
        'Import failed',
        "The import you requested has failed due to a system error. Please contact your system administrator to investigate.\n\nRegards\nIndicia Team"
      );
    }
    else {
  ...
  private static function notifyUserOfValidationErrorsAndCleanup($db, $task, $params) {
    ...
    if ($personInfo) {
      ...
      $emailer->send(
        ...
        "The import you requested has failed validation.\n<a href=\"$link\">Download the rows that had errors</a>\n\nRegards\nIndicia Team"
      );
    }
    else {
