...
class rest_spreadsheet_verify {
  ...
  private static function checkNextBatch(array &$metadata) {
    ...
    for ($i = $offset - 1; $i < $offset - 1 + ROWS_PER_BATCH; $i++) {
      ...
      $providedStatus = isset($row[$metadata['statusColIndex']]) ? $row[$metadata['statusColIndex']] : NULL;
      $providedComment = isset($row[$metadata['commentColIndex']]) ? $row[$metadata['commentColIndex']] : NULL;
      if (empty($providedStatus) && empty($providedComment)) {
        // No status or comment update in this row so skip it.
        continue;
      }
      if (!empty($providedStatus)) {
   ...
   */
  private static function processNextBatch(array &$metadata) {
