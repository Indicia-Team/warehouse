ALTER TABLE taxon_associations
  ADD fidelity smallint,
  ADD CONSTRAINT chk_taxon_associations_fidelity CHECK (fidelity BETWEEN 1 AND 3);

COMMENT ON COLUMN taxon_associations.fidelity
    IS 'Fidelity of the association - 1 = high, 2 = medium, 3 = low.';
...
select
  ...
  cttlfrom.taxon_list_id=cttlto.taxon_list_id as same_list, cttlfrom.website_id, ta.fidelity
...
select
  ...
  ta.created_by_id, c.username AS created_by, ta.updated_by_id, u.username AS updated_by, cttlfrom.website_id, ta.fidelity
from taxon_associations ta
join cache_taxa_taxon_lists cttlfrom on cttlfrom.taxon_meaning_id=ta.from_taxon_meaning_id
join cache_taxa_taxon_lists cttlto on cttlto.taxon_meaning_id=ta.to_taxon_meaning_id
join cache_termlists_terms atype on atype.id=ta.association_type_id
