...
class Plant_Portal_Import_Controller extends Service_Base_Controller {
  ...
  public function upload() {
    ...
    if (file_exists($csvTempFile))
    {
      ...
      while (($data = fgetcsv($handle, 1000, ",")) !== FALSE && ($limit===false || $count<$limit)) {
        ...
        //Commented out for now as Plant Portal is not going to use associations in the immediate future
  ...
  * records.
  *
  * @param array $saveArray Array to wrap
  ...
  */
  /*private static function plant_portal_wrap($model,$saveArray, $fkLink = false)
  {
    ...
    $r = submission_builder::build_submission($saveArray, $model->get_submission_structure());
      // Map fk_* fields to the looked up id
    if ($fkLink) {
    ...
    if (array_key_exists('superModels', $r)) {
      ...
      foreach ($r['superModels'] as $super) {
        $r['superModels'][$idx]['model'] = $model->getFkFields($super['model'], $saveArray);
        $r['superModels'][$idx]['model']=self::plant_portal_intercept_Fk_fields($r['superModels'][$idx]['model'], $saveArray);
        $idx++;
      }
    }
    return $r;
  ...
  private static function plant_portal_intercept_Fk_fields($singleSubmissionLevel, $saveArray) {
    ...
    foreach ($singleSubmissionLevel['fkFields'] as $fkFieldData) {
      if ($fkFieldData['fkIdField']==='location_id') {
        unset($singleSubmissionLevel['fkFields']);
        foreach ($singleSubmissionLevel['fields'] as $field=>&$value) {
          //if (substr($field, 0, 3)=='fk_') {
          if ($field==='fk_location') {
            $reportOptions = array(
              'dataSource'=>'reports_for_prebuilt_forms/plant_portal/get_plots_from_groups_for_user',
              'readAuth'=>$readAuth,
              //Need to swap it hard-coded test values here for now
              'extraParams' => array('plot_group_permission_person_attr_id'=>,'user_id'=>,'plot_name'=>'Test Plot Name','plot_sp_ref'=>'<test sref goes here>','plot_group_name'=>null));
            $locationData = data_entry_helper::get_report_data($reportOptions);
            if (!empty($data[0]['name']))
              $value['value']=$data[0]['name'];
            else
              kohana::log('debug', 'Warehouse could not find plot in user\'s plot groups.');
          }
        }
        
      }
    }
    return $singleSubmissionLevel;
  }*/
}