 ...
 * Queue worker to update cache_occurrences_nonfunctional.attrs_json.
 *
 * Class called when a task_cache_builder_attr_value_occurrence task
 * encountered in the work queue after a direct update on an attribute value.
 * Updates cache_occurrences_nonfunctional.attrs_json with a json attribute for
 * easy reporting on attribute values.
 */
class task_cache_builder_attr_value_occurrence {
  ...
  public static function process($db, $taskType, $procId) {
    ...
    if ($langs !== NULL) {
      foreach ($langs as $lang) {
        ...
        $langTermSql .= <<<SQL
          ...
          FROM work_queue q
          JOIN occurrence_attribute_values avfilt ON avfilt.id=q.record_id
          JOIN occurrence_attribute_values av ON av.occurrence_id=avfilt.occurrence_id AND av.deleted=false
          ...
          WHERE q.entity='occurrence' AND q.task='task_cache_builder_attr_value_occurrence' AND claimed_by=$procIdEsc
          AND a.data_type='L'
    ...
    $sql = <<<SQL
      ...
      FROM (
        SELECT avfilt.occurrence_id, a.multi_value,
        ...
        LEFT JOIN terms t ON t.id=tlt.term_id AND t.deleted=false
        WHERE q.entity='occurrence_attribute_value' AND q.task='task_cache_builder_attr_value_occurrence' AND claimed_by=$procIdEsc
        GROUP BY avfilt.occurrence_id, av.occurrence_attribute_id, a.multi_value
        $langTermSql
      ) AS subquery
