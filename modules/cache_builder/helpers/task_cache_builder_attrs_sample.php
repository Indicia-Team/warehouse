  ...
  public static function process($db, $taskType, $procId) {
    ...
    if ($langs !== NULL) {
      foreach ($langs as $lang) {
        ...
        $langTermSql .= <<<SQL
          ...
          WHERE q.entity='sample' AND q.task='task_cache_builder_attrs_sample' AND claimed_by=$procIdEsc
          AND a.data_type='L'
          GROUP BY q.record_id, av.sample_attribute_id, a.multi_value
    ...
    $sql = <<<SQL
      ...
      FROM (
        ...
        FROM work_queue q
        LEFT JOIN sample_attribute_values av ON av.sample_id=q.record_id AND av.deleted=false
          AND COALESCE(av.int_value::text, av.text_value::text, av.float_value::text, av.date_start_value::text) IS NOT NULL
