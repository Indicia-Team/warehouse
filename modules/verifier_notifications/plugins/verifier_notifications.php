...
function verifier_notifications_process_task_type($type, $params, $db, $runOverdueCheckInstead) {
  ...
  foreach ($urls as $url) {
    ...
    // Get all filters where the user for the filter does not already have an unacknowledged notification  of that type
...
/*
 * If we aren't using overdue occurrences then we only need to run task when occdelta is populated (new or updated occurrences).
 * If checking overdue occurrences then it needs running anyway regardless of occdelta as occurrence can become overdue without being updated
 */
