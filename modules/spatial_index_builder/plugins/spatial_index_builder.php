...
function spatial_index_builder_metadata() {
  // Don't skip this plugin as the scheduled tasks runner does not take into
  // account changed locations.
  return array(
    'always_run' => TRUE,
  );
}
 ...
 *
 * @param string $last_run_date
 *   Timestamp when this was last run, used to get DB changed records.
 * @param object $db
 *   Database object.
 *
 * @return int
 ...
 *
 * @return array
 *   Array containing the join SQL in the first entry and where SQL in the
 *   second.
 */
 ...
 * @param $filter
 * @param string $limit
 *   Sql to limit to the updated locations or samples.
 */
...
function spatial_index_builder_add_to_cache($db) {
  ...
  foreach ($types as $type) {
    // We can only do this type of indexing for boundary types that occur only
    // once per sample.
    if (!in_array($type['term'], $config['unique']))
      continue;
    // Script for handling updated samples can be constructed to do all
    // location types in one go.
...
JOIN;
    // Script for handling updated locations is a bit more complex so we have
    // to run once per location type.
    $db->query(<<<QRY
