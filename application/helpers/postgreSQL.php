...
class postgreSQL {
   ...
   */
  public static function transformWkt($wkt, int $fromSrid, int $toSrid, $db = NULL) {
    if ($fromSrid != $toSrid) {
      if (!$db) {
        $db = new Database();
      }
      $result = $db->query("SELECT ST_asText(ST_Transform(ST_GeomFromText(?, ?), ?)) AS wkt;", [$wkt, $fromSrid, $toSrid])->current();
      return $result->wkt;
    }
    else {
      return $wkt;
   ...
   * subsequent comments.
   */
  public static function selectVerificationAndCommentNotifications($lastRunDate, $maxTime, $db) {
    ...
    $lastRunDate = pg_escape_literal($db->getLink(), $lastRunDate);
    $maxTime = pg_escape_literal($db->getLink(), $maxTime);
    return $db->query(<<<SQL
-- autogenerated record cleaner comments (except identification difficulty)
select distinct 'A' as source_type, co.id, co.created_by_id as notify_user_id, cttl.taxon, cttl.language_iso, cttl.preferred_taxon, cttl.default_common_name, co.date_start, co.date_end, co.date_type, snf.public_entered_sref,
...
-- autogenerated record cleaner comments for identification difficulty
select distinct 'A' as source_type, co.id, co.created_by_id as notify_user_id, cttl.taxon, cttl.language_iso, cttl.preferred_taxon, cttl.default_common_name, co.date_start, co.date_end, co.date_type, snf.public_entered_sref,
        co.verified_on, oc.comment, true as auto_generated, oc.generated_by, co.record_status, co.record_substatus, co.updated_on, oc.created_by_id as occurrence_comment_created_by_id,
        oc.generated_by as source_detail, 't' as record_owner
from cache_occurrences_functional co
...
-- verified record for the same taxon.
...
select distinct 'V', co.id, co.created_by_id as notify_user_id, cttl.taxon, cttl.language_iso, cttl.preferred_taxon, cttl.default_common_name, co.date_start, co.date_end, co.date_type, snf.public_entered_sref,
...
where co.created_by_id<>1 and co.verified_on between $lastRunDate and $maxTime and co.confidential=false
and co.training=false
union
...
left join occurrence_comments oc2 on oc2.occurrence_id=co.id and oc2.deleted=false and oc2.created_on between $lastRunDate and $maxTime
...
select distinct 'C' as source_type, co.id, ocprev.created_by_id as notify_user_id, cttl.taxon, cttl.language_iso, cttl.preferred_taxon, cttl.default_common_name, co.date_start, co.date_end, co.date_type, snf.public_entered_sref,
...
SQL
   ...
   */
  public static function selectPendingGroupsUsersNotifications($lastRunDate, $db = NULL) {
    if (!$db) {
      $db = new Database();
    }
    // Note this query excludes user 1 from the notifications (admin user) as
    // they are records which don't have a warehouse user ID.
    return $db->query(<<<SQL
...
SQL;
      ...
      foreach ($smpInfo as $s) {
        ...
        foreach ($sizes as $size) {
          ...
          $existing = $db
            ->query("SELECT id FROM map_squares WHERE x=? AND y=? AND size=?", [
              $squareDetail['x'],
              $squareDetail['y'],
              $squareDetail['size'],
            ])
            ->result_array(FALSE);
          if (count($existing) === 0) {
            $qry = $db->query("INSERT INTO map_squares (geom, x, y, size) VALUES (reduce_precision(st_geomfromtext(?, ?), ?, ?), ?, ?, ?)", [
              $s->geom,
              $srid,
              $s->confidential,
              $squareDetail['size'],
  ...
  private static function taxonSearchCheckOptions($db, array &$options) {
    ...
    self::stringListOption($db, $options, 'search_code');
  ...
  private static function taxonSearchGetQueryContextFilter(array $options) {
    ...
    if (!empty($options['min_taxon_rank_sort_order'])) {
      $filters[] = 'cts.taxon_rank_sort_order >= ' . (int) $options['min_taxon_rank_sort_order'];
    }
      ...
      elseif (preg_match('/\*[^\s]/', strtolower($searchTerm))) {
        ...
        $likesearchterm = pg_escape_string($db->getLink(),
          preg_replace(
            '/[^a-zA-Z0-9%\+\?*]/',
            '',
            str_replace(['*', ' '], '%', str_replace('ae', 'e', preg_replace('/\(.+\)/', '', strtolower($searchTerm))))
          ) . '%'
        );
        $searchFilters[] = "(cts.simplified=true and searchterm like /***/ '$likesearchterm')";
        $highlightRegex = pg_escape_string($db->getLink(), '(' . preg_replace([
...
SQL;
   ...
   *     search to (e.g. limit to a list of taxa having the same accepted name
   *     TVK).
   *   * search_code - Search code or array of search codes to limit the
   *     search to (e.g. limit to a list of taxa having the exact TVKs).
