...
class Termlists_term_Controller extends Gridview_Base_Controller {
  ...
  /**
   * Gets the index grid action columns.
   *
  ...
  private function internal_index($termlist_id) {
    // No further filtering of the gridview required as the very fact you can
    // access the parent termlist means you can access all the taxa for it.
  ...
  /**
   * Retrieve model values to load onto the edit form.
   *
   * Returns an array of all values from this model and its super models ready to be
   * loaded into a form and the synonyms/common names.
   */
  protected function getModelValues() {
    $r = parent::getModelValues();
    $this->loadAttributes($r, ['termlist_id' => [$this->model->termlist_id]]);
  ...
  protected function getDefaults() {
    ...
    $this->loadAttributes($r,
      array('termlist_id' => [$r['termlists_term:termlist_id']])
    );
    return $r;
  }
   ...
   *
   * @param int $id
   *   Id of the termlists_term that is being checked, or null for a new record.
   */
