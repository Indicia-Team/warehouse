...
foreach ($lists as $list) {
  $rec = $this->db->select('id')
      ->from('termlists_termlists_term_attributes')
      ->where(array(
        'termlist_id'=>$list->id,
        'termlists_term_attribute_id'=>html::initial_value($values,$model->object_name.':id'),
        'deleted'=>'f'
      ))
      ->get()->result_array();
  echo '<li><label for="termlist_'.$list->id.'" class="wide">'.$list->title.'</label>';
  echo form::checkbox('termlist_'.$list->id, TRUE, count($rec)>0, 'class="vnarrow"');
  echo '</li>';
}
?>
  </ol>
