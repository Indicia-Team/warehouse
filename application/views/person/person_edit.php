...
<form id="entry_form" action="<?php echo url::site() . 'person/save'; ?>" method="post">
  ...
  <legend>Custom attributes</legend>
    <ol>
      ...
      foreach ($attrsWithMulti as $personAttributeId => $wholeAttrToDraw) {
        ...
        if (!empty($wholeAttrToDraw['caption'])) {
          handle_single_value_attributes('psnAttr', $personAttributeId, $wholeAttrToDraw, $values);
        } else {
          handle_multi_value_attributes('psnAttr', $personAttributeId, $wholeAttrToDraw, $values);
        }
      }
      ?>
    </ol>
