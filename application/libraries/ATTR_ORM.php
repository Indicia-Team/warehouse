...
abstract class ATTR_ORM extends ORM {
	...
	public function validate(Validation $array, $save = FALSE) {
		...
		$this->multi_value = $array['multi_value'];
	...
	public function preSubmit() {
		...
		if ($this->submission['fields']['disabled_input']['value'] == 'NO') {
			if (!is_numeric($this->submission['fields']['termlist_id']['value']))
				$this->submission['fields']['termlist_id']['value'] = NULL;
			if (isset($this->submission['fields']['multi_value']))
				$this->submission['fields']['multi_value'] = array('value' => 't');
			else
				$this->submission['fields']['multi_value'] = array('value' => 'f');
			if (isset($this->submission['fields']['public']))
				$this->submission['fields']['public'] = array('value' => 't');
			else
				$this->submission['fields']['public'] = array('value' => 'f');
			
			// ToDo: In the form need to constrain possible validation rules dependant on data type.
