...
class ReportEngine {
  ...
  /**
   * A list mappings from known custom attribute captions to the IDs.
   *
   * @var array
   */
   ...
   *   Format of the report file. Currently only xml report file formats are supported.
   * @param array $params
   *   Associative array of report parameters.
   * @param boolean $resultAsArray
   *   Set to FALSE to return the result as a pg result object rather than an array.
   */
   ...
   *
   * @param boolean $resultAsArray
   *   TRUE to return the result as an array, FALSE to return a pg result object which needs to be iterated (better for
   *   large results set since it doesn't load all data into memory).
   *
   * @return array
  ...
  private function compileReport($resultAsArray) {
    ...
    if (!empty($unpopulatedParams)) {
      // Send a request for further parameters back to the client. If the request specified the list of parameters to
   ...
   *
   * Takes the data and columns lists, and carries out any post query processing. This includes vague date processing,
   * and any other defined by the report reader.
   */
   ...
   *
   * Takes the data and columns lists, and looks for a vague date column set.  If one is found, inserts a new column for
      ...
      foreach ($attrData as $row) {
        ...
        if (!array_key_exists($row[$attributeDefn->id], $newColumns)) {
          ...
          switch ($row["data_type"]) {
            case 'D':
            case 'V':
              $this->columns[$newColName . "_date_start"] = array(
                'display' => $row[$attributeDefn->caption] . " Start",
                'class' => '',
                'style' => '',
                'autodef' => ($vagueDateProcessing && $attributeDefn->hideVagueDateFields == 'true')
              );
              $this->columns[$newColName . "_date_end"] = array(
                'display' => $row[$attributeDefn->caption] . " End",
                'class' => '',
                'style' => '',
                'autodef' => ($vagueDateProcessing && $attributeDefn->hideVagueDateFields == 'true')
              );
              $this->columns[$newColName . "_date_type"] = array(
                'display' => $row[$attributeDefn->caption] . " Type",
                'class' => '',
                'style' => '',
                'autodef' => ($vagueDateProcessing && $attributeDefn->hideVagueDateFields == 'true')
              );
              // If vague date processing enable for the report, add the extra column.
              if ($vagueDateProcessing) {
                $this->columns[$newColName . "_date"] = array(
...
SQL;
  ...
  /**
   * The following function is the only method by which the reports can update the contents of the database. As a
   * consequence the following restrictions are enforced:
   * 1. The updates are not data driven. The only thing updated is the downloaded column in the occurrences table, and
   *    this is hardcoded.
   */
    ...
    foreach ($this->providedParams as $name => $value) {
      if (isset($paramDefs[$name])) {
        if ($value === '') {
          ...
          // Empty integer params should be handled as 0 (NULL would be ideal, but we can't test for it in the same
          // fashion as a number).
   ...
   *
   * Returns true if the report is using standard parameters and is limited to a group ID which allows its members to
   * view sensitive records, only if the current user is a group member.
   */
   ...
   * @param string $entity
   *   Either occurrence, location or sample depending on the type of attributes being loaded.
   * @param string $attrList
   *   Parameter value, which should be a comma separated list of attribute IDs or attribute names.
   * @param string $idx
   *   An optional index number when dealing with parent/child table arrangements.
   *
  ...
  private function mergeAttrListParam($query, $entity, $attrList, $idx = "") {
    ...
    // Find an attribute that supports survey filtering as this allows us to retrieve the specific set of attributes
    // for the survey.
    ...
    if ($surveys) {
      if (isset($websiteIds)) {
        ...
        // May have come from warehouse frontend where no website provided, but a survey has been provided.
        // Need attributes_websites table.
        $this->reportDb
          ->join("{$entity}_attributes_websites as aw", "aw.{$entity}_attribute_id", 'a.id')
          ->where(array('aw.deleted' => 'f'));
        $this->reportDb->where("aw.restrict_to_survey_id in ($surveys)");
    ...
    // Don't sort if getting all: Note that there isn't a sample_method or location_type style restriction on the attributes fetched.
    $newAttrList = array();
   ...
   *
   * Returns the field name that needs to be picked out of the attribute values or terms table for a particular data
   * type.
   *
   * @param string $entity
   ...
   * @param boolean $forceVarchar
   *   Set true if there are mixed datatypes, so non-text will be forced to varchar.
   *
   * @return string
   *   Field name.
   */
   ...
   *
   * Create the joins and column definitions required to support a set of custom attributes added to the report by
   * specifying one or more system functions.
   *
   * @param string $query
  ...
  private function processSysfuncAttrs(&$query, $entity, $attrs, $idx) {
    ...
    if ($this->reportReader->loadStandardParamsSet === 'occurrences' || $this->reportReader->loadStandardParamsSet === 'samples') {
      ...
      $done = array();
      $sysfuncsList = array();
      // Find all the system functions in the list of attributes we've been given. Prepare some metadata required for
      // each sys func's set of joins.
      foreach ($attrs as $attr) {
        if (!empty($attr->system_function)) {
          ...
          // Store the ids associated with each type of data for each sysfunc so we can efficiently build SQL to
          // output the value.
   ...
   *
   * @return array
   *   Array keyed by list of column names, with qualifying suffix so add to column caption if there are multiple.
   */
  ...
  private function processStandardAttrs(&$query, $entity, $attrs, $usingCaptions, $idx) {
    ...
    foreach ($attrs as $attr) {
      ...
      // We use the attribute ID or the attribute caption to create the unique column alias, depending on how it was
      // requested.
   ...
   *
   * @return string
   *   Either LEFT JOIN (optional attributes) or JOIN (required attributes) so that further related joins can be
   *   constructed.
   */
