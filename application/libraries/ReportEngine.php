...
/**
 * @file
 * Report engine, provides access to predefined or ad-hoc report metadata and output data.
 *
 ...
 *
 * * Grab the report and parse it for parameters.
 * * Send a response back to the requester, inviting them to fill in the parameters. This
 *   reponse will include the id generated in step 3.
 * * The requester sends back the requested parameters, which are checked against the cache to
 *   ensure they're all there. If not, repeat these steps.
 * * The core reparses the report, merges the parameters in and executes the query
 *   against the core database. Results are formatted and returned to the requester.
 *
...
class ReportEngine {
  ...
  /**
   * If the record count has been queried, store it so we don't ask twice.
   *
   * @var integer
   */
  ...
  private function add_vague_dates(&$data) {
    ...
    foreach ($colSets as $colSet) {
      for ($r = 0; $r < $dataCount; $r++) {
        $row = $data[$r];
        if (!empty($row[$colSet . 'date_type'])) {
          try {
            $data[$r][$colSet . 'date'] = vague_date::vague_date_to_string(array(
              $row[$colSet . 'date_start'],
              $row[$colSet . 'date_end'],
              $row[$colSet . 'date_type']
            ));
          }
          catch (Exception $e) {
            kohana::log('error', 'Error in report vague date conversion: ' . $e->getMessage());
            $data[$r][$colSet . 'date'] = 'Invalid';
          }
        }
      }
    }
  ...
  private function updateDownloaded($idList, $mode) {
    ...
    // Use default database object for access so can update.
    $db = new Database();
    $db->query('START TRANSACTION READ WRITE;');
    $response = $db
      ->in("id", $idList)->where("downloaded_flag", ($mode == 'FINAL' ? 'I' : 'N'))
      ->update('occurrences',
        array(
  ...
  private function fetchProvidedReport($request) {
    ...
    if (is_dir($uploadDir)) {
      ...
      if (file_put_contents($uploadDir . $fname, $request)) {
        ...
        // @todo Handle case where cannot write to temp file.
      }
    }
    else {
   ...
   *   Current query ORDER BY clause
   *
   * @return string
   *   Altered ORDER BY clause.
   *
  ...
  /**
   * Handle the addition of a system function attribute column.
   *
   ...
   *
   * @return array
   *   Array keyed by list of column names, with qualifying suffix so add to column caption if there are
   *   multiple.
   */
