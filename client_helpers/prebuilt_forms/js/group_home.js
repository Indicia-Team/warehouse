...
jQuery(document).ready(function($) {
  ...
  add_comment = function(id,date,entered_sref,record_status,created_by_id) {
    $.fancybox(
      '<form>' +
        '<label>Comment:</label>'+
        '<textarea id="occurrence-comment" rows="4"></textarea><br>'+
        '<input onclick="occurrence_comment_submit('+id+','+ "'" + date + "'" +','+ "'" + entered_sref + "'" +','+ "'" + record_status + "'" +','+created_by_id+')" type="button" value="Submit">' +
      '</form>'
    );
  }
