...
/**
   ...
   */
  public static function get_control_species($auth, $args, $tabalias, $options) {
    global $user;
...
//    file_put_contents('/var/www/vhosts/monitoring.wwt.org.uk/httpdocs/recording/tmp/debug.txt',print_r($subjectObservations,true));
   ...
   */
  public static function get_control_observationcomment($auth, $args, $tabalias, $options) {
    $fieldPrefix = !empty($options['fieldprefix']) ? $options['fieldprefix'] : '';
    return data_entry_helper::textarea(array_merge(array(
      'fieldname'=>$fieldPrefix.'subject_observation:comment',
      'label'=>lang::get('Any information you might like to record'),
...
public function rfj_individual_imagediv($taxIdx){
...
if (isset(data_entry_helper::$entity_to_load[$fieldprefix.'subject_observation:parent_id'])) 
 ...
 if(!$parent_id && !$next_parent_id ) return 'NEITHER';
  ...
  protected static function get_control_speciesidentifier($auth, $args, $tabalias, $options) {
    static $taxIdx = 0; 
//fail if any missing attributes
self::rfj_check_speciesidentifier_attributes_present(wwt_individual::$options);   
//somehow the following is re-including tabs content into first page
self::rfj_check_speciesidentifier_javascript(wwt_individual::$options,wwt_individual::$args); //modifies passed vars
$r="";
$sampleFamilies=array();
$sampleSingles=array(); //in terms of known individuals
	$numIndivs=count(self::$subjectObservationIds);

	for($taxIdx=0;$taxIdx<$numIndivs;$taxIdx++){
	$indiv=new wwt_individual($taxIdx,$auth,$args,$options,$tabalias); //family needs to be before indivs 



if($indiv->isFamily()){ //$r.="\nIs family $taxIdx\n";
	if(isset($sampleFamilies[$indiv->getSjoId()])) {// was already created by individual record
		$indiv->addIndivs($sampleFamilies[$indiv->getSjoId()]->getIndivs()); // update to include prior indivs
		$sampleFamilies[$indiv->getSjoId()]=$indiv;
		}
	$sampleFamilies[$indiv->getSjoId()]=$indiv;
}
else if ($indiv->isPartOfFamily()){//$r.="\nIn family $taxIdx\n";
	if(!isset($sampleFamilies[$indiv->getParentId()])){
	$sampleFamilies[$indiv->getParentId()]=new wwt_individual(-1); 
	}
	$sampleFamilies[$indiv->getParentId()]->addIndivs($indiv);
}
else	{//$r.="\nNot family $taxIdx\n";
$sampleSingles[$indiv->getSjoId()]=$indiv;
}

}

foreach($sampleFamilies as $sampleFamily){
$r.=$sampleFamily->getSectionHeader();
$r.="Family".$sampleFamily->getSjoId()."<br/>";

foreach($sampleFamily->getIndivs() as $indiv){
$r.=$indiv->getIndividualHeader();
$r.=$indiv->showSpecies();
$r.=$indiv->showIdentifiers($taxIdx,$tabalias);
...
$r.=$indiv->getIndividualFooter();
}
$r.=$indiv->getSectionFooter();
}


foreach($sampleSingles as $sampleSingle){
$r.=$sampleSingle->getSectionHeader();
$r.="Single".$sampleSingle->getSjoId()."<br/>";
$r.=$sampleSingle->getIndividualHeader();
$r.=$sampleSingle->showSpecies();
$r.=$sampleSingle->showIdentifiers($taxIdx,$tabalias);
// subject_observation comment
if ($args['observation_comment']) {
     $r .= self::get_control_observationcomment($auth, $args, $tabalias, $options);
}
...
$r.=self::rfj_image_upload($taxIdx,$tabalias,array());
//$r.=self::rfj_image_upload($taxIdx,$tabalias,$options);
$r.=$sampleSingle->getIndividualFooter();
$r.=$sampleSingle->getSectionFooter();
}
...
$r.=self::setupIdentifiers($options,$r,$args,$taxIdx,$auth,$tabalias);    // output each required identifier
...
if ($args['observation_comment']) {
  ...
  */
  ...
  return "";
  // Currently a problem with javascript tab handling of multiple file uploads
$r='<div></div>';
    $opts = array(
    ...
    $opts['imageWidth'] = '168';
    $opts['id'] = 'idn:'.$taxIdx;
    $opts['uploadSelectBtnCaption'] = lang::get('Uploading photo(s)');
    if ($options['inNewIndividual']) {
...
//      $r .= '</fieldset>';  
  ...
  public static function get_control_identifier($auth, $args, $tabalias, $options) {
...
//    $r.=$grid; // add normal grid
  ...
  ### Problem with merging here is that duplicates are appended, not merged, due to numeric keys - these can only be additional parameters!!!
   ...
   public function setupIdentifiers(&$options,$args,$taxIdx,$auth,$tabalias) {   
    // output each required identifier
    $r = '<div id="'.$options['fieldprefix'].'accordion" class="idn-accordion">';

$r.=self::displayIdent($options,$auth, $args, $tabalias,"neck_collar","neck-collar","collar",$taxIdx,'Neck');
$r.=self::displayIdent($options,$auth, $args, $tabalias,"enscribed_colour_ring","colour-ring","colourRing",$taxIdx,'Colour Leg Ring');
$r.=self::displayIdent($options,$auth, $args, $tabalias,"metal_ring","metal","metalRing",$taxIdx,'Metal Ring');
//self::displayIdent($options,$auth, $args, $tabalias,"enscribed_colour_ring","colour-ring","colourRing",$taxIdx,'Colour Leg Ring 2');



// Metal ring needs to be a choice for legs instead
//$r.=::displayIdent($options,$auth, $args, $tabalias,"metal_ring","metal","metalRing",$taxIdx);
...
public function displayIdent($options,$auth, $args, $tabalias,$identType,$identName,$identFormat,$taxIdx,$marktype){
...
/*    
  protected static function get_control_speciesidentifier_off($auth, $args, $tabalias, $options) {
    static $taxIdx = 0; 
...
$r.=self::get_control_speciesidentifier2($auth, $args, $tabalias, $options);
