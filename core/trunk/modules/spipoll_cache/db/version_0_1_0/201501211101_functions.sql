...
DROP TRIGGER IF EXISTS run_cache_occurrences    ON occurrences CASCADE;
DROP TRIGGER IF EXISTS run_cache_determinations ON determinations CASCADE;
DROP TRIGGER IF EXISTS run_cache_locations      ON locations CASCADE;
DROP TRIGGER IF EXISTS run_cache_samples        ON samples CASCADE;
DROP TRIGGER IF EXISTS run_cache_sample_attribute_values ON sample_attribute_values CASCADE;
...
CREATE OR REPLACE FUNCTION spipoll_get_determination_details(dettype character, detuser text, detdate date) RETURNS text AS $$
DECLARE
	retVal	text;
BEGIN
...
CREATE OR REPLACE FUNCTION spipoll_get_taxon_details(tid integer, tidList integer[]) RETURNS text AS $$
DECLARE
	retVal	text;
	rtaxon	text;
	tid1	integer;
...
CREATE OR REPLACE FUNCTION rebuild_spipoll_collection(arg_collection_id integer) RETURNS integer AS $$
DECLARE
 count integer;
 collectionrow samples%ROWTYPE;
BEGIN
 count := 0;
 FOR collectionrow IN SELECT * FROM samples
 WHERE parent_id IS NULL AND deleted = false AND id = arg_collection_id
 LOOP
  BEGIN
   perform update_spipoll_cache_entry(collectionrow, true);
   count := count + 1;
...
BEGIN
  ...
  ELSIF TG_RELNAME='determinations' THEN
   ...
   END IF;
   RETURN NEW;
  ELSIF TG_RELNAME='occurrences' THEN
   SELECT * INTO collectionRow FROM samples WHERE id=NEW.sample_id AND survey_id = surveyID;
   IF FOUND THEN
    --- only if insect
    IF collectionRow.parent_id IS NOT NULL THEN
     sessionRow := collectionRow;
     SELECT * INTO collectionRow FROM samples WHERE id=sessionRow.parent_id AND survey_id = surveyID AND parent_id IS NULL;
     IF FOUND THEN
      perform update_spipoll_cache_entry(collectionRow, false);
...
CREATE TRIGGER run_cache_occurrences AFTER DELETE OR UPDATE ON occurrences FOR EACH ROW EXECUTE PROCEDURE check_run_cache();
CREATE TRIGGER run_cache_determinations AFTER INSERT OR DELETE OR UPDATE ON determinations FOR EACH ROW EXECUTE PROCEDURE check_run_cache();
...
--- When the sample is deleted, the sample is modified as well as the closed SAV being set.
CREATE TRIGGER run_cache_samples AFTER UPDATE ON samples FOR EACH ROW EXECUTE PROCEDURE check_run_cache();
