...
CREATE OR REPLACE VIEW list_identifier_attribute_values AS 
 SELECT pav.id, p.id AS identifier_id, pa.id AS identifier_attribute_id, 
   ...
   JOIN identifier_attributes pa ON (pa.id = COALESCE(pav.identifier_attribute_id, paw.identifier_attribute_id) OR pa.public = true) AND (pa.id = pav.identifier_attribute_id OR pav.id IS NULL) AND (pa.id = paw.identifier_attribute_id OR pa.public = true) AND pa.deleted = false
...
-- ALTER TABLE list_identifier_attribute_values OWNER TO indicia_user;
...
CREATE OR REPLACE VIEW list_subject_observation_attribute_values AS 
 SELECT pav.id, p.id AS subject_observation_id, pa.id AS subject_observation_attribute_id, 
   ...
   JOIN subject_observation_attributes pa ON (pa.id = COALESCE(pav.subject_observation_attribute_id, paw.subject_observation_attribute_id) OR pa.public = true) AND (pa.id = pav.subject_observation_attribute_id OR pav.id IS NULL) AND (pa.id = paw.subject_observation_attribute_id OR pa.public = true) AND pa.deleted = false
   LEFT JOIN (termlists_terms tt
   JOIN terms t ON t.id = tt.term_id
   JOIN languages l ON l.id = t.language_id) ON tt.meaning_id = pav.int_value AND pa.data_type = 'L'::bpchar
  WHERE p.deleted = false
...
-- ALTER TABLE list_subject_observation_attribute_values OWNER TO indicia_user;
