...
class extension_splash_extensions {
  ...
  /*
   * Display a linked pair of select lists consisting of a broad habitat scale and a finer scale sub-control.
   * The fine scale list consists of child terms of the main list.
   */
  public static function fine_scale_habitats($auth, $args, $tabAlias, $options) {
    if (empty($options['habitatAttributeId'])) {
      drupal_set_message('Please fill in the @habitatAttributeId option for the habitat control');
      return '';
    }
    $extraParams=array('habitat_attr_id'=>$options['habitatAttributeId']);
    If (!empty($_GET['sample_id']))
      $extraParams=array_merge($extraParams,array('sample_id'=>$_GET['sample_id']));      
    //Firstly get broad habitats from report
    $reportOptions = array(
      'dataSource'=>'reports_for_prebuilt_forms/Splash/get_broad_and_fine_habitats',
      'readAuth'=>$auth['read'],
      'mode'=>'report',
      'extraParams' => $extraParams
    );
    $rawData = data_entry_helper::get_report_data($reportOptions);
    //Cycle through each habitat
    foreach($rawData as $rawRow) {
      //If the habitat doesn't have a parent, then it means that it is a parent itself and therefore a broad scale habitat.
      if (empty($rawRow['parent_id']))
        $broadHabitatsData[$rawRow['id']]=$rawRow['term'];  
      //If the report returns an existing habitat termlists_term id then we know we are in edit mode and it has previously been saved.
      if (!empty($rawRow['existing_fine_habitat_id'])) {
        //In the case we have an existing value for the fine habitat then the broad habitat needs to be set to the parent
        if ($rawRow['id']===$rawRow['existing_fine_habitat_id']) {
          $defaultBroadHabitat=$rawRow['parent_id'];
          $defaultFineHabitat=$rawRow['existing_fine_habitat_id'];
          //We need the sample_attribute_value id, as it needs to be attached to the html name so the system knows to override that value.
          $defaultFineHabitatAttrValueId=$rawRow['existing_fine_habitat_attr_value_id'];
        }
      }
    }
    //Options for broad habitat drop-down
    //Note this is not actually saved to the database, when reloading in edit mode the broad habitat is worked out from the fine habitat
    $selectListOptions=array(
      'id' => 'broad-habitat-select-list',
      'blankText'=>'<Please select>',
      'fieldname'=> 'broad-habitat-select-list',
      'label' => lang::get('Select a broad habitat'),
      'helpText' => lang::get('Select a broad habitat and then a fine-scale habitat.'),
      'lookupValues' => $broadHabitatsData,
      'captionField'=>'term',
    );
    //In edit mode, the default broad habitat needs setting
    if (!empty($defaultBroadHabitat)) 
      $selectListOptions=array_merge($selectListOptions,array('default'=>$defaultBroadHabitat));
    $r = data_entry_helper::select($selectListOptions);
    //Then get the fine scale habitat options
    $extraParams = array_merge($auth['read'], $extraParams);
    $selectListOptions=array(
      'id' => 'fine-habitat-select-list',
      'blankText'=>'<Please select>',
      'label' => lang::get('Select a fine scale habitat'),
      'parentControlId'=> 'broad-habitat-select-list',
      'filterField'=> 'parent_id',
      'captionField'=>'term',
      'valueField'=>'id',
      'reportProvidesOrderBy'=>true,
      'report'=>'reports_for_prebuilt_forms/Splash/get_broad_and_fine_habitats',
      'extraParams'=>$extraParams,
    );
    //In edit mode, then set a default for the field and setup the html to include the sample_attribute_value
    if (!empty($defaultFineHabitat)) 
      $selectListOptions=array_merge($selectListOptions,array('default'=>$defaultFineHabitat,'fieldname'=>'smpAttr:'.$options['habitatAttributeId'].':'.$defaultFineHabitatAttrValueId));
    else 
      $selectListOptions=array_merge($selectListOptions,array('fieldname'=> 'smpAttr:'.$options['habitatAttributeId']));
    $r .= data_entry_helper::select($selectListOptions);
    return $r; 
  }
}
