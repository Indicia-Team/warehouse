...
class iform_group_edit {
  ...
  public static function get_form($args, $node, $response=null) {
    ...
    if (!empty($args['group_type'])) {
      ...
      $groupType=strtolower($response[0]['term']);
    } else {
      $groupType=lang::get('group');
    }
    ...
    $r .= data_entry_helper::text_input(array(
      'label' => lang::get('{1} name', ucfirst($groupType)),
      'fieldname'=>'group:title',
      'validation'=>array('required'),
      'class'=>'control-width-5',
      'helpText'=>lang::get('The full title of the {1}', $groupType)
    ));
    if ($args['include_code'])
      $r .= data_entry_helper::text_input(array(
        ...
        'helpText'=>lang::get('A code or abbreviation identifying the {1}', $groupType)
      ));
    if (empty($args['group_type'])) {
    ...
    if (count($joinMethods)===1) {
      ...
      $r .= data_entry_helper::radio_group(array(
        'label' => ucfirst(lang::get('{1} membership', $groupType)),
    ...
    $r .= data_entry_helper::textarea(array(
      'label' => ucfirst(lang::get('{1} description', $groupType)),
      'fieldname' => 'group:description',
      'helpText' => lang::get('Description and notes about the {1}.', $groupType)
    ));
    if ($args['include_dates']) {
      $r .= data_entry_helper::date_picker(array(
        'label' => ucfirst(lang::get('{1} active from', $groupType)),
      ...
      $r .= data_entry_helper::date_picker(array(
        ...
        'labelClass' => 'auto',
        'helpText' => lang::get('Specify the period during which the {1} was active.', $groupType)
      ));
    }
    if ($args['include_private_records']) 
      $r .= data_entry_helper::checkbox(array(
        ...
        'helpText'=>lang::get('Tick this box if you want to withold the release of the records from this {1} until a '.
          'later point in time, e.g. when a project is completed.', $groupType)
      ));
    if ($args['include_administrators']) {
      $r .= data_entry_helper::sub_list(array(
        'fieldname'=>'groups_user:user_id',
        'label' => ucfirst(lang::get('{1} administrators', $groupType)),
        'table'=>'user',
    ...
    $r .= '<input type="submit" class="indicia-button" id="save-button" value="'.
        ...
        lang::get('Create {1}', $groupType) : lang::get('Update {1} settings', $groupType))
        ."\" />\n";    
    $r .= '</form>';
    if (isset($hiddenStuff))
      $r .= $hiddenStuff;
...
$('#entry_form').submit(function() {
  ...
  public static function get_submission($values, $args) {
    ...
    $existingAdmins=preg_grep('/^groups_user\:user_id\:[0-9]+$/', array_keys($values));
    if (!empty($values['groups_user:user_id']) || !empty($existingAdmins)) {
      $s['subModels']=array();
      if (!empty($values['groups_user:user_id'])) {
        foreach($values['groups_user:user_id'] as $userId) {
          $s['subModels'][]=array('fkId' => 'group_id', 
            'model' => submission_builder::wrap(array('user_id'=>$userId, 'administrator'=>'t'), 'groups_user'));
        }
      }
  ...
  private static function loadExistingGroup($id, $auth, $args) {
    ...
    if ($args['include_administrators']) {
      $admins = data_entry_helper::get_population_data(array(
        'table'=>'groups_user',
        'extraParams'=>$auth['read']+array('view'=>'detail', 'group_id'=>$_GET['group_id'], 'administrator'=>'t'),
        'nocache'=>true
      ));
      $arr = array();
      foreach($admins as $user) {
        $arr[]=array(
            'fieldname'=>'groups_user:user_id:'.$user['id'],
            'caption'=>$user['person_name'],
            'default'=>$user['user_id']
        );
      }
      data_entry_helper::$entity_to_load['groups_user:user_id']=$arr;
    }
  }
