...
class Location_Attribute_Value_Model extends ORM {
  ...
  public function validate(Validation $array, $save = FALSE) {
    ...
    $array->add_rules('location_attribute_id', 'required');
    $array->add_rules('location_id', 'required');
    $this->unvalidatedFields = array(
      'deleted'
    );
    
    // We apply the validation rules specified in the location attribute
    // table to the value given.
    if (array_key_exists('location_attribute_id', $array->as_array())) {
      $id = $array->as_array();
