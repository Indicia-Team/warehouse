...
?>
<p>This page allows you to specify a new custom attribute for an <?php echo $other_data['name']; ?>.
When creating a new custom attribute, you can either select from a list
of available attributes to attach to other websites or surveys, or build
...
<input type="hidden" name="metaFields:disabled_input"
	value="<?php echo $disabled_input; ?>" /> <?php echo $values['attribute_load'] ?>
<fieldset
...
<ol>
	<li><label for="caption">Caption</label> <input id="caption"
		...
		<?php echo $values['enabled']; ?> /> <?php echo html::error_message($model->getError('custom_attribute:caption')); ?>
	</li>
...
<?php echo $values['enabled']; ?>
	...
	<li><label for="termlist_id">Termlist</label> <select id="termlist_id"
		name="custom_attribute:termlist_id" <?php echo $values['enabled']; ?>>
		<option value=''>&lt;Please Select&gt;</option>
		<?php
	...
	<?php echo form::checkbox('custom_attribute:multi_value', TRUE, (html::initial_value($values, 'custom_attribute:multi_value') == 't'), 'class="vnarrow" '.$values['enabled'] ) ?>
	</li>
	<li><label class="wide" for="public">Available to other Websites</label>
	<?php echo form::checkbox('custom_attribute:public', TRUE, (html::initial_value($values, 'custom_attribute:public') == 't'), 'class="vnarrow" '.$values['enabled'] ) ?>
	</li>

	<li><b>Validation Rules:</b></li>
	<li id="li_valid_required"><label class="narrow" for="valid_required">Required</label><?php echo form::checkbox('valid_required', TRUE, isset($model->valid_required) AND ($model->valid_required == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_length"><label class="narrow" for="valid_length">Length</label><?php echo form::checkbox('valid_length', TRUE, isset($model->valid_length) AND ($model->valid_length == 't'), 'class="vnarrow" '.$values['enabled'] ) ?><input
		class="narrow" id="valid_length_min" name="valid_length_min"
		value="<?php echo html::specialchars($model->valid_length_min); ?>"
		<?php echo $values['enabled']?> /> - <input class="narrow"
		id="valid_length_max" name="valid_length_max"
		value="<?php echo html::specialchars($model->valid_length_max); ?>"
		<?php echo $values['enabled']?> /> <?php echo html::error_message($model->getError('valid_length')); ?>
	</li>
	<li id="li_valid_alpha"><label class="narrow" for="valid_alpha">Alphabetic</label><?php echo form::checkbox('valid_alpha', TRUE, isset($model->valid_alpha) AND ($model->valid_alpha == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_email"><label class="narrow" for="valid_email">Email
	Address</label><?php echo form::checkbox('valid_email', TRUE, isset($model->valid_email) AND ($model->valid_email == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_url"><label class="narrow" for="valid_url">URL</label><?php echo form::checkbox('valid_url', TRUE, isset($model->valid_url) AND ($model->valid_url == 't'), 'class="vnarrow" '.$values['enabled']  ) ?></li>
	<li id="li_valid_alpha_numeric"><label class="narrow"
		for="valid_alpha_numeric">Alphanumeric</label><?php echo form::checkbox('valid_alpha_numeric', TRUE, isset($model->valid_alpha_numeric) AND ($model->valid_alpha_numeric == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_numeric"><label class="narrow" for="valid_numeric">Numeric</label><?php echo form::checkbox('valid_numeric', TRUE, isset($model->valid_numeric) AND ($model->valid_numeric == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_standard_text"><label class="narrow"
		for="valid_standard_text">Standard Text</label><?php echo form::checkbox('valid_standard_text', TRUE, isset($model->valid_standard_text) AND ($model->valid_standard_text == 't'), 'class="vnarrow" '.$values['enabled'] ) ?></li>
	<li id="li_valid_decimal"><label class="narrow" for="valid_decimal">Formatted
	Decimal</label><?php echo form::checkbox('valid_decimal', TRUE, isset($model->valid_decimal) AND ($model->valid_decimal == 't'), 'class="vnarrow" '.$values['enabled'] ) ?><input
		class="narrow" id="valid_dec_format" name="valid_dec_format"
		value="<?php echo html::specialchars($model->valid_dec_format); ?>"
		<?php echo $values['enabled']?> /> <?php echo html::error_message($model->getError('valid_decimal')); ?>
	</li>
	<li id="li_valid_regex"><label class="narrow" for="valid_regex">Regular
	Expression</label><?php echo form::checkbox('valid_regex', TRUE, isset($model->valid_regex) AND ($model->valid_regex == 't'), 'class="vnarrow" '.$values['enabled'] ) ?><input
		class="narrow" id="valid_regex_format" name="valid_regex_format"
		value="<?php echo html::specialchars($model->valid_regex_format); ?>"
		<?php echo $values['enabled']?> /> <?php echo html::error_message($model->getError('valid_regex')); ?>
	</li>
	<li id="li_valid_min"><label class="narrow" for="valid_min">Minimum
	value</label><?php echo form::checkbox('valid_min', TRUE, isset($model->valid_min) AND ($model->valid_min == 't'), 'class="vnarrow" '.$values['enabled'] ) ?><input
		class="narrow" id="valid_min_value" name="valid_min_value"
		value="<?php echo html::specialchars($model->valid_min_value); ?>"
		<?php echo $values['enabled']?> /> <?php echo html::error_message($model->getError('valid_min')); ?>
	</li>
	<li id="li_valid_max"><label class="narrow" for="valid_max">Maximum
	value</label><?php echo form::checkbox('valid_max', TRUE, isset($model->valid_max) AND ($model->valid_max == 't'), 'class="vnarrow" '.$values['enabled'] ) ?><input
		class="narrow" id="valid_max_value" name="valid_max_value"
		value="<?php echo html::specialchars($model->valid_max_value); ?>"
		<?php echo $values['enabled']?> /> <?php echo html::error_message($model->getError('valid_max')); ?>
	</li>
</ol>
</fieldset>
