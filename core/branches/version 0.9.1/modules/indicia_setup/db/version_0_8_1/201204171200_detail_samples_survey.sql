...
CREATE OR REPLACE VIEW detail_samples AS 
 SELECT s.id, s.entered_sref, s.entered_sref_system, s.geom, st_astext(s.geom) AS wkt, s.location_name, s.date_start, s.date_end, s.date_type, 
     ...
     su.website_id, s.parent_id, s.comment, s.recorder_names, su.id as survey_id, su.title as survey_title, s.sample_method_id
   FROM samples s
