-- adds taxon group : allows filtering using this detail view rather than list view
...
CREATE OR REPLACE VIEW detail_taxa_taxon_lists AS 
 SELECT ttl.id, ttl.taxon_id, t.taxon, t.authority, l.iso AS language_iso, tc.taxon AS common, ttl.taxon_list_id, tl.title AS taxon_list, ttl.taxon_meaning_id, 
     ...
     ti.path AS image_path, ti.caption AS image_caption, ttl.created_by_id, c.username AS created_by, ttl.updated_by_id, u.username AS updated_by, t.external_key, ttl.allow_data_entry, tg.title AS taxon_group
   ...
   LEFT JOIN taxon_groups tg ON tg.id = t.taxon_group_id
