...
CREATE OR REPLACE VIEW list_locations AS 
 SELECT l.id, l.name, l.code, l.centroid_sref, lw.website_id, l.location_type_id
   FROM locations l
   LEFT JOIN locations_websites lw ON l.id = lw.location_id
  WHERE l.deleted = false;

CREATE OR REPLACE VIEW detail_locations AS 
 SELECT l.id, l.name, l.code, l.comment, l.parent_id, p.name AS parent, l.centroid_sref, l.location_type_id,
     l.centroid_sref_system, st_astext(l.centroid_geom) AS centroid_geom, st_astext(l.boundary_geom) AS boundary_geom, 
