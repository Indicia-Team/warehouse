...
$custom_terms = array(
	...
	'LANG_DE_Instructions2'=>"You may add new sites by choosing either the polygon or line control and then drawing the new sites' boundaries on the map, clicking on each point, and double clicking on the final point in each element to complete it. You may then add another line or polygon. The new sites will be added to the list of sites for this square, and given a default number. You can change the number of a new site using the 'Site number' field below. After adding a drawn element, you will notice some small red circles appear around the boundary you have just drawn: you can change the element by dragging these circles. To delete a point, place the mouse over the red circle, and press the 'Delete' button on the keyboard. Further new sites may be added by first clicking on the 'Start a new site' button, and then repeating the process.<br />Only one site's boundary will be modifiable at any one time. Should you wish to modify another new site, select the 'Click on the map to select a site' control, and then click on the site on the map. Pick the control for the type of drawn element you wish to modify. You will see the small red circles appear around the relevant elements for that site.<br />If you create a site, but do not save any data against it, it will NOT be recorded in the database.<br />It is not possible to change a site name or boundary on this form once it has been saved - this can be done by an Admin user using their special tool.",
	'LANG_LocModTool_Instructions2'=>"Either click on the map to select the site you wish to modify, or choose from the drop down list. You may then change its name, or its extent on the map by dragging the red vertices. To delete a vertex, place the mouse over the vertex and press the 'Delete' button.",
