...
class iform_wwt_colour_marked_clone  extends iform_dynamic_sample_occurrence{
  ...
  public static function get_wwt_colour_marked_clone_definition() {
    return array(
      'title'=>'RFJ Colour-marked Wildfowl',
...
public static function get_perms($nid) {
   ...
   * Get the map control.
   */
  protected static function get_control_map_off($auth, $args, $tabalias, $options) {
    $options = array_merge(
...
//                }
   ...
   */
  protected static function getSpeciesNameFilterMode_off($args) {
    if (isset($args['species_names_filter'])) {
      switch ($args['species_names_filter']) {
        case 'language':
   ...
   */
   //RFJ disabled so that dynamic sample class used instead 06/12/2012
  protected static function build_grid_autocomplete_function_off($args) {
    global $indicia_templates;  
   ...
   * autocomplete_include_both_names and autocomplete_include_taxon_group.
   */
  protected static function build_grid_taxon_label_function_off($args) {
    global $indicia_templates;  
   ...
   */
  protected static function get_control_samplecomment_off($auth, $args, $tabalias, $options) {
   ...
   */
  protected static function get_control_date_off($auth, $args, $tabalias, $options) {
   ...
   */
  protected static function get_control_locationname_off($auth, $args, $tabalias, $options) {
   ...
   */
  protected static function get_control_recordernames_off($auth, $args, $tabalias, $options) {
    return data_entry_helper::textarea(array_merge(array(
   ...
   */
  protected static function get_control_recordstatus_off($auth, $args) {    
  ...
  private static function get_control_speciesidentifier($auth, $args, $tabalias, $options) {
      ...
      if (!empty($sjoAttributeType['system_function'])) {
        switch ($sjoAttributeType['system_function']) {
          ...
          case 'life_status' :
            ...
            break;
          case 'assoc_id' :
            $options['assocId'] = $sjoAttributeType['id'];
            break;
   ...
   $r .= '<fieldset id="'.$options['fieldprefix'].'family:fieldset" class="taxon_family taxon_family0 ui-corner-all">';
    $r .= '<legend id="'.$options['fieldprefix'].'family:legend">Family details</legend>';
/* Don't include individual header - include in fieldset*/
/*    if ($args['subjectAccordion']) {
    ...
    // bird image div 
    $r .= '<div title="<b>Mouse button clicks:</b><br/><b>Left:</b> type the ring code,<br/><b>middle:</b> toggle text colour<br/><b>right:</b> cycle through ring colours" id="'.$options['fieldprefix'].'image:panel" class="image_panel ui-corner-all beautytips">';
   
   
   
    $r .= '<div id="idn:'.$taxIdx.':neck-collar:colourbox" class="identifier neck-collar-identifier-colourbox"><input value="c" class="identifier-colourbox-input neck-collar-identifier-colourbox-input ui-corner-all"></div>';
   
    $r .= '<div id="idn:'.$taxIdx.':colour-ring:colourbox" class="identifier colour-ring-identifier-colourbox"><input value="b" class="identifier-colourbox-input colour-ring-identifier-colourbox-input ui-corner-all"></div>';
   
    $r .= '<div id="idn:'.$taxIdx.':metal:colourbox" class="identifier metal-identifier-colourbox"><input value="a" class="identifier-colourbox-input metal-identifier-colourbox-input ui-corner-all"></div>';
    ...
    if (isset(data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:id'])) {
      $r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:id" name="'.$options['fieldprefix'].'subject_observation:id" '.
    ...
    {
      $r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:parent_id" name="'.$options['fieldprefix'].'subject_observation:parent_id" '.
'value="'.data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:parent_id'].'" />'."\n";    
    }
    else{
$r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:parent_id" name="'.$options['fieldprefix'].'subject_observation:parent_id" '.'value="" />'."\n";    
    }
    

//Try unknown adults - rfj
if (isset(data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:unmarked_adults']))
    {
      $r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:unmarked_adults" name="'.$options['fieldprefix'].'subject_observation:unmarked_adults" '.
'value="'.data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:unmarked_adults'].'" />'."\n";    
    }
    else{
      $r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:unmarked_adults" name="'.$options['fieldprefix'].'subject_observation:unmarked_adults" '.
'value="22" />'."\n";    
//$r.='<div>'.print_r($options,true).'</div>';	
    }
$r.=data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:unmarked_adults'];
$r.=data_entry_helper::$entity_to_load[$options['fieldprefix'].'subject_observation:unmarked adults'];
    ...
    if ($value!=='') {
      $r .= '<input type="text" id="'.$options['fieldprefix'].'subject_observation:count" '.
...
self::setupIdentifiers($options,$r,$args,$taxIdx,$auth,$tabalias);
...
//$options['attrList'][]=array('attrType' => 'idn', 'typeId' => $options['positionId'], 'lockable' => false, 'hidden' => $hidepos, /*'hiddenValue' => $args['left_'.$identType.'_position'],*/);
//$options['attrList'][]=array('attrType' => 'idn', 'typeId' => $options['baseColourId'], 'lockable' => !$hidecolour, 'hidden' => $hidecolour,);
//$options['attrList'][]=array('attrType' => 'idn', 'typeId' => $options['textColourId'], 'lockable' => !$hidecolour, 'hidden' => $hidecolour,);
//$options['attrList'][]=array('attrType' => 'idn', 'typeId' => $options['sequenceId'], 'lockable' => false, 'hidden' => false,);
