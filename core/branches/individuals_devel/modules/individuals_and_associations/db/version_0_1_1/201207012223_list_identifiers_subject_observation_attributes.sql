-- View: list_identifiers_subject_observation_attributes

-- DROP VIEW list_identifiers_subject_observation_attributes;

CREATE OR REPLACE VIEW list_identifiers_subject_observation_attributes AS 
...
'::text) || pa.validation_rules::text, COALESCE(paw.validation_rules, ''::character varying)::text || COALESCE(pa.validation_rules, ''::character varying)::text) AS validation_rules, pa.deleted, paw.deleted AS website_deleted, pa.public
   ...
   LEFT JOIN identifiers_subject_observation_attributes_websites paw ON pa.id = paw.identifiers_subject_observation_attribute_id AND paw.deleted = false
